...
def train_parser():
    ...
    parser.add_argument("--opt",help="optimizer",default="sgd",choices=['adam','sgd'])
    ...
    parser.add_argument("--weight_decay",help="weight decay for optimizer",default=0.0005,type=float)
    parser.add_argument("--gpu",help="gpu device",type=int,default=0)
    parser.add_argument("--seed",help="random seed",type=int,default=42)
    parser.add_argument("--val_epoch",help="number of epochs before eval on val",type=int,default=50)
    ...
    parser.add_argument("--no_val", help="don't use validation set, just save model at final timestep",default=False,action="store_true")
    parser.add_argument("--train_way",help="training way",type=int,default=15)
    ...
    parser.add_argument("--test_transform_type",help="size transformation type during inference",type=int,default=0)
    parser.add_argument("--val_trial",help="number of meta-testing episodes during validation",type=int,default=1000)
    parser.add_argument("--detailed_name", help="whether include training details in the name",action="store_true",default=False)
    parser.add_argument("--lamda", type=float, default=0.5)
    parser.add_argument("--save_file_path", type=str, default="/home/akun648/projects/MS_Freq_Net/result")
    parser.add_argument("--dataset", type=str, default="cub", choices=['cub','cars','dogs',"FD-PDC"])
    parser.add_argument("--resnet", help="whether use resnet12 as backbone or not",default=True,action="store_true")
...
class Train_Manager:
    ...
    def __init__(self,args,path_manager,train_func):
        ...
        files_name = str(current_time) + "_" + str(name)
        file = args.save_file_path
        file_path = os.path.join(file, files_name)
        ...
        self.logger = get_logger(os.path.join(file_path, '%s.log' % (name)))
        self.save_path = os.path.join(file_path, 'model_%s_.pth' % (name))
        self.writer = SummaryWriter(os.path.join(file_path, 'log_%s' % (name)))
    ...
    def train(self,model):
        ...
        for e in tqdm(range(total_epoch)):
            ...
            iter_counter,train_acc,avg_loss = train_func(model=model,
                                                optimizer=optimizer,
                                                writer=writer,
            ...
            if (e+1)%args.val_epoch==0:
                ...
                logger.info("train_loss: %.3f" % (avg_loss))
